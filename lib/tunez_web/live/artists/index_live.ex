defmodule TunezWeb.Artists.IndexLive do
  # PHOENIX LIVEVIEW FOR ARTIST LISTING
  # ====================================
  ...
  # MOUNT CALLBACK
  # ==============
  # Initial setup - sets page title
  # Data loading deferred to handle_params for URL parameter handling
  ...
  # ======================
  # Loads artist data using Ash's generated functions
  # Called on initial load and when URL params change
  def handle_params(params, _url, socket) do
    sort_by = Map.get(params, "sort_by") |> validate_sort_by()
    query_text = Map.get(params, "q", "")
    # ASH INTEGRATION: Using generated read function
    # Music.read_artists! is generated by:
    #   define(:read_artists, action: :read)
    #
    # Returns a list of all artists
    # The ! version raises on error
    #
    # FUTURE ENHANCEMENTS with Ash:
    # - Add filtering: Music.read_artists!(filter: [name: "Beatles"])
  ...
  def pagination_links(assigns) do
    ...
    class="flex justify-center pt-8 space-x-4">
      <.button_link data-role="previous-page" kind="primary" inverse patch={~p"/?#{query_string(@page, @query_text,@sort_by, "prev")}"} disabled={!AshPhoenix.LiveView.prev_page?(@page)}>
        « Previous
      </.button_link>
      <.button_link data-role="next-page" kind="primary" inverse patch={~p"/?#{query_string(@page, @query_text,@sort_by, "next")}"} disabled={!AshPhoenix.LiveView.next_page?(@page)}>
        Next »
      </.button_link>
    </div>
    """
  ...
  def handle_event("search", %{"query" => query}, socket) do
    # Example: Music.read_artists!(filter: [name: [ilike: "%#{query}%"]])
