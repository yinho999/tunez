defmodule TunezWeb.Artists.ShowLive do
  ...
  def handle_event("destroy-artist", _params, socket) do
    case Tunez.Music.destroy_artist(socket.assigns.artist) do
      ...
      {:error, error} ->
        Logger.info("Couldnt delete artist '#{socket.assigns.artist.id}' #{inspect(error)}")

        socket =
          socket
          |> put_flash(:error, "Couldnt delete the Artist")
  ...
  def handle_event("destroy-album", %{"id" => album_id}, socket) do
    case Tunez.Music.destroy_album(album_id) do
      ...
      {:error, error} ->
        Logger.info("Couldnt delete album #{album_id}: #{inspect(error)}")
        socket = socket |> put_flash(:error, "Couldnt delete album")
        {:noreply, socket}
    end
  end
